...
"""       
# boto3 lambda invoke syntax for reference
...
def load_txt(genre, testing_bool):
    ...
    if testing_bool == True:
        ...
        for line in album_file_r:
            splits = line.strip().split()
           # arr = np.array([float(x) for x in splits[1:]])
...
#####################
def load_txtNOTUSING(genre, testing_bool):
    """
    Loads vectors from a text file.
    :param testing_bool: if True, loads local .txt testing file
                         else, loads from S3 bucket
    ...
    if testing_bool == True:
        ...
        for line in album_file_r:
            ...
            vectors[splits[0]] = np.array([float(x) for x in splits[1:]])
            vector_array.append(np.array([float(x) for x in splits[1:]]))
    return vectors, vector_array

def get_recsNOTUSING(user_vector, genre):
    testing_bool=False
    vectors, vector_array = load_txt(genre, testing_bool)
    recs = []
    #user_answers = get_user_vector(answers)
    user_answers = user_vector
    N_KNN = 2
    knn = NearestNeighbors(n_neighbors=N_KNN)
    knn.fit(vector_array)  # instead of z
    neighbor_radii, neighbors = knn.kneighbors([user_answers], N_KNN, return_distance=True)  # get nearest neighbors to point
    neighbors = neighbors[0][1:]  # get inner list of neighbors without counting node itself as its own neighbor
    print("neighbors:", neighbors)
    for index in neighbors:
        recs.append(neighbors[index][0])
    return recs
##################### 
   
# SWITCHOVER  
def get_recs(user_vector, genre, username, email):    
#def get_recs(user_vector, genre):
    testing_bool = False
    ...
    print("neighbors:", neighbors)
    for index in neighbors:
        #recs.append(list(vector_array[index]))
